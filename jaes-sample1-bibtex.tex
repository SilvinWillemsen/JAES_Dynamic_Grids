...
artefacts when changing between grid configurations for lossy systems, though this needs to be confirmed by listening tests. Some insights into stability analysis and real-time implementation are provided. 
...
Both for compact implementation of FD schemes as well as for applying the dynamic grid to more complex systems later on, it is useful to write an update equation (such as Eq. \eqref{eq:1DwaveUpdate}) in matrix form. The general matrix form of an update equation is defined as
...
The grid locations $\lu = \Mu$ and $\lw = 0$  are referred to as the \textit{inner boundaries}, at which Eq. \eqref{eq:splitFDS} (after expansion) requires a definition for the \textit{virtual grid points} $u_{\Mu+1}^n$ and $w_{-1}^n$. If $\Nfrac^n = N^n$, i.e. $\Nfrac^n$ is an integer, the inner boundaries overlap and need to abide the following condition
...
\end{equation}
Using this definition causes Eq. \eqref{eq:splitFDS} and Eq. \eqref{eq:1dWaveDisc} to exhibit identical behaviour.

Consider a decrease in wave speed $c^n$, which yields a decrease in $h^n$. This causes all grid points to move towards their respective outer boundary according to Eq. \eqref{eq:gridLocations} (see Figure \ref{fig:twoFreeStringsGridMove}). As the inner boundaries no longer overlap, i.e., $\Nfrac^n \neq N^n$, Eq. \eqref{eq:perfectOverlap} can no longer be used and alternative definitions for the virtual grid points need to be found. To this end, quadratic Lagrange interpolation can be used to yield:
...
\end{equation}
where the definitions of $\AA$, $\BB$ and $\CC$ depend on the system at hand. Furthermore, $\MoneD \times 1$ vector
\begin{equation}\label{eq:uStack}
    \uStack^n = \begin{bmatrix}
    \u^n, \w^n
    \end{bmatrix}^T,
\end{equation}
with $\MoneD = \Mu + \Mw$, includes the state of the subsystems in Eq. \eqref{eq:splitFDS}. Here, $\u^n = [u_1^n, u_2^n, \hdots, u_{\Mu}^n]^T$ and $\w^n = [w_0^n, w_1^n, \hdots, w_{\Mw-1}^n]^T$ which are $\Mu \times 1$ and $\Mw \times 1$ respectively. 
...
If $c^n$ is decreased such that $N^n > N^{n-1}$, a grid point is added to the system. One can add points to either $\u$ or $\w$, or to both in an alternating fashion as in \cite{Willemsen2021a}, but here, only changes in $\u$ are considered. A grid point can be added to $\u$ according to
\begin{equation}
\begin{aligned}
...
The limit on changing grid configurations is the addition / removal of one grid point per sample. Empirical studies confirmed that this limit needs to be much lower to keep the system well-behaved. %In \cite{Willemsen2021b}, a lower limit of 20 samples per change is used together with $\lambda = 0.999$ and yields no noticeable artefacts.
...
\begin{subequations}
\begin{align}
    \pt q &\approxeq \dtd \qln \triangleq \frac{1}{2k}\left(q_l^{n+1} - q_l^{n-1}\right),\\
    \pt q &\approxeq \dtm \qln \triangleq \frac{1}{k}\left(\qln- q_l^{n-1}\right),
\end{align}
\end{subequations}
Eq. \eqref{eq:stiffString} can be discretised using the following FD scheme:
...
\end{aligned}
\end{equation}
where $\mu^n = \kappa^n k/(h^n)^2$ and (for simply supported boundary conditions) $\DDxxxx = \DDxx\DDxx$. Notice that $\kappa^n$, $\sigma_0^n$ and $\sigma_1^n$ are now allowed to be time-varying. Furthermore, $h^n$ can be obtained by calculating the stability condition with equality. %Also note that damping terms have been excluded for brevity, but can be added trivially.
...
Only considering alterations in the left subsystems, a column can be added by carrying out the following operation on $\v$ in Eq. \eqref{eq:vw}
\begin{equation}
\begin{aligned}
...
Only considering alterations in the top subsystems, a row can be added by carrying out the following operation on $\u_1$ and $\u_2$ in Eq. \eqref{eq:2DuDefs} 
\begin{equation*}
    \begin{aligned}
...
\begin{align}
    ...
    \pyy q &\approxeq \dyy \qlmn \triangleq \frac{1}{h^2}\left(q_{l,m+1}^n - 2 \qlmn + q_{l,m-1}^n\right).
\end{align}
\end{subequations}
...
\begin{aligned}
    ...
    \BB &= 2\I_\MtwoD - (\mu^n)^2 \DDDeltaDelta + \frac{2\sigma_1^nk}{(h^n)^2}\DDDelta,\\
...
% \end{equation}
where $h^n = 2\sqrt{\kappa^n k}$, and for simply supported boundary conditions $\DDDeltaDelta = \DDDelta\DDDelta$, and is of size $\MtwoD \times \MtwoD$. Sound examples can be found via \cite{soundExamples}.
...
Results of the stiff string with $\kappa \approx 1.26$ m$^2$/s and changes in $c$ corresponding to $\Nfrac = 15 \rightarrow 20$ can be seen in Fig. \ref{fig:resultsStiffString}. The location where grid points are added / removed does not influence the modal frequencies. 
...
\begin{figure}[t]
    ...
    \caption{Modal frequencies of the stiff string in Eq. \eqref{eq:stiffStringDynMatrix} with $\kappa \approx 1.26$ and changes in $c$ corresponding to $\Nfrac = 15 \rightarrow 20$. Modes exhibited by the system are shown in solid black, and expected modal frequencies are shown using dashed red lines.}
...
Results for the 2D case can be found in Table \ref{tab:2dresults}.
...
\SWcomment[This section is still a work in progress..]
With reference to Fig. \ref{fig:resultsStiffString}, consider a stiff string with $\Nfrac = 15$ excited at $t=0$, and $c$ is changed such that $\Nfrac = 20$. If a grid point is added, and modal frequencies of the new grid configuration align with that of the previous, the energy of these modes get transferred. This means that every time a grid point, and thus a mode is added at the top of the spectrum, it will not obtain any energy. If the system is excited at a later point, these modes will naturally be excited too.
Furthermore, no noticeable artefacts occur when adding grid points (at sub-audio rate parameter variations). 

If, on the other hand, $\Nfrac = 20\rightarrow 15$ (effectively horizontally flipping Fig. \ref{fig:resultsStiffString}), modes seem to `disappear' around the Nyquist frequency when grid points are removed. This, as mentioned in Sec. \ref{sec:addRemove}, could cause audible artefacts. Informal experiments show that if modes are damped before they `disappear', noticeable artefacts are prevented. In the case of the stiff string, this is what frequency-dependent losses automatically allow for. %If parameters are changed such that grid points are removed, noticeable artefacts are prevented for a high enough value $\sigma_1$. 
The minimum value that $\sigma_1$ needs to have to prevent artefacts depends on various factors (e.g., values of other parameters, speed of parameter variation, etc.), and further experiments will need to be done to find this. 

In the case of 2D systems, multiple grid points, and thus modes, `disappear' at once. Although these do not only the highest frequency modes, damping does take care of noticeable artefacts. 
...
Although it is not ideal to rely on (frequency-dependent) losses for the method to work, it is more natural than to add artificial damping at specific locations in the grid. Furthermore, these losses occur naturally in any physical material. 
...
One could potentially extend this method to 3D, performing another Kronecker sum as Eq. \eqref{eq:dynamicKronecker} for the 3rd dimension (see \cite{Hamilton2016}).
