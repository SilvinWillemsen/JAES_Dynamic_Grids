...
In the following, the location of a grid point $q_l$ at time index $n$ will be written as $x_{q_l}^n$ (in m from the left boundary). Moreover, the following time-varying parameters are indicated with a superscript $n$: $L^n$, $c^n$, $h^n$, $\Nfrac^n$ and $N^n$. % and $f_0^n
As a starting point, the original system $\qln$, with $l=\{0, \hdots, N^n\}$ is split into two subsystems: $\ulun$ with $l_u=\{0, \hdots, \Mu\}$ \SBcomment[Why $M^{n}$ and not $M_{u}^{n}$?] \SWcomment[$M^n$ is used a lot for the inner boundary index (see e.g. Eqs. \eqref{eq:connectionInterpol}) so I thought it'd be more compact to leave out the subscript. Should I add it for coherency?]\SBcomment[Or just use a different capital letter for the right grid...it's weird to have one with a subscript and one without.] and $\wlwn$ with  $l_w=\{0, \hdots, \Mw\}$:
...
To evaluate the dynamic grid several analyses have been done. 
